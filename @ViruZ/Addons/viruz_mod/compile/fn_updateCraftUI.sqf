...
_init = {
	...
	uiNamespace setVariable ["VIRUZ_CraftingDialog_Items", _itemArray];
};

_cleanup = {
	if (dialog) then { closeDialog 0 };
	uiNamespace setVariable ["VIRUZ_CraftingDialog", nil];
	uiNamespace setVariable ["VIRUZ_CraftingDialog_Items", nil];
};
...
_craftItem = {
	...
	if (count _classList > 0) then {
		...
		[_arrayToSend, _tipo] call player_checkRecipe;
...
_display = uiNamespace getVariable ["VIRUZ_CraftingDialog", nil];
_itemArray = uiNamespace getVariable ["VIRUZ_CraftingDialog_Items", []];
