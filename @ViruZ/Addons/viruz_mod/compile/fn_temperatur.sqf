...
*/
	...
	//water
	if(surfaceIsWater getPosATL player || viruz_isSwimming) then {
	...
	_difference = _difference * SleepTemperatur / (60 / _looptime)		* ((viruz_temperaturmax - viruz_temperaturmin) / 100);
	
	//Change Temperatur															 Should be moved in a own Function to allow adding of Items which increase the Temp like "hot tea"
	viruz_temperatur = (((viruz_temperatur + _difference) max viruz_temperaturmin) min viruz_temperaturmax);
	
	//Add Shivering
	//						Percent when the Shivering will start 
	if(viruz_temperatur <= (0.125 * (viruz_temperaturmax - viruz_temperaturmin) + viruz_temperaturmin)) then {
		//CamShake as linear Function Maximum reached when Temp is at temp minimum. First Entry = Max Value
		_temp = 0.6 * (viruz_temperaturmin / viruz_temperatur );
