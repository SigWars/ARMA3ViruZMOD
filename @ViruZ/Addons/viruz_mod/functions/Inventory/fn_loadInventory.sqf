...
if !("uniform" in _blacklist) then {
	...
	if (_isCfg) then {
		...
		if (typename _uniform == typename []) then {_uniform = _uniform call bis_fnc_selectrandom;};
	};
	if (_uniform != "") then {
...
if !("vest" in _blacklist) then {
	if (typename _vest == typename []) then {_vest = _vest call bis_fnc_selectrandom;};
	if (_vest != "") then {
...
if !("headgear" in _blacklist) then {
	if (typename _headgear == typename []) then {_headgear = _headgear call bis_fnc_selectrandom;};
	if (_headgear != "") then {
...
if !("goggles" in _blacklist) then {
	if (typename _goggles == typename []) then {_goggles = _goggles call bis_fnc_selectrandom;};
	if (_goggles != "") then {
...
if !("backpack" in _blacklist) then {
	...
	if (_isCfg) then {
		...
		if (typename _backpack == typename []) then {_backpack = _backpack call bis_fnc_selectrandom;};
	};
	if (_backpack != "") then {
...
if !("magazines" in _blacklist) then {
	if (_isCfg) then {
		...
		{
			if (count _x > 2) then {
				if !((_x select 2) isEqualTo "") then {_object addmagazine (_x select 2);};
			};
		} foreach [_inventory select 6,_inventory select 7,_inventory select 8];
	};
};
if !("weapons" in _blacklist) then {
	private ["_weapons"];
...
if !(_isCfg) then {
	...
	if !("uniform" in _blacklist) then {
		{
			if (typename _x == typename []) then {
				(uniformContainer _object) addMagazineAmmoCargo [(_x select 0), 1, (_x select 1)];
			} else {
				if (_x != "") then {
	...
	if !("vest" in _blacklist) then {
		{
			if (typename _x == typename []) then {
				(vestContainer _object) addMagazineAmmoCargo [(_x select 0), 1, (_x select 1)];
			} else {
				if (_x != "") then {
	...
	if !("backpack" in _blacklist) then {
		{
			if (typename _x == typename []) then {
				(backpackContainer _object) addMagazineAmmoCargo [(_x select 0), 1, (_x select 1)];
			} else {
				if (_x != "") then {
