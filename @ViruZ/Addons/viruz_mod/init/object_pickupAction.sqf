	...
	while { _run } do {
		if (alive _holder) then {
			...
			if ((_distance < 1.75) and (_action == -1)) then {
				_action = player addAction [format[(localize "STR_viruz_mod_1"),_name], "\z\addons\viruz_mod\actions\object_pickup.sqf",[_type,_classname,_holder], 20, true, true];
			...
			if (_distance > 100) then {
				null = _holder addAction [format[(localize "STR_viruz_mod_1"),_name], "\z\addons\viruz_mod\actions\object_pickup.sqf",[_type,_classname,_holder], 20, true, true];
				player reveal _holder;
				_run = false;
				_timeout = 0;
			};
