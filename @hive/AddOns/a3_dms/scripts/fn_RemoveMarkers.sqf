...
if (_status == "win") then
{
	...
	//[DMS_MissionMarkerWinDotTime, {deleteMarker (_this select 0);}, [_markerDot], false] call TRASHServer_system_thread_addTask;
	if (DMS_DEBUG) then
	{
...
else
{
	...
	//[DMS_MissionMarkerLoseDotTime, {deleteMarker (_this select 0);}, [_markerDot], false] call TRASHServer_system_thread_addTask;
	if (DMS_DEBUG) then
	{
